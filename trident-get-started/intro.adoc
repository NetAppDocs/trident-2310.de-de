...
Um eine Testversion zu nutzen, kann Zugriff auf „Einfache Bereitstellung und Klonung von persistentem Speicher für containerisierte Workloads“ link:https://www.netapp.com/test-drives/["NetApp Testversion"^] mit einem sofort einsatzbereiten Lab-Image angefordert werden. Die Testversion bietet eine Sandbox-Umgebung mit einem Kubernetes-Cluster mit drei Knoten, in dem Astra Trident installiert und konfiguriert ist. Dies ist eine gute Möglichkeit, sich mit Astra Trident vertraut zu machen und dessen Funktionen zu erkunden.
