...
[role="lead"]
Astra Trident kann entweder manuell oder mithilfe von Helm mit dem Trident operator aktualisiert werden. Ein Upgrade von einer Trident operator Installation auf eine andere Trident operator Installation oder von einer  `tridentctl` Installation auf eine Trident operator Version ist möglich. link:upgrade-trident.html#step-3-select-an-upgrade-method["Wählen Sie eine Aktualisierungsmethode aus"] sollte vor dem Upgrade einer Trident operator Installation geprüft werden.
...
NOTE: Für ein Upgrade von Astra Trident, das mit dem Namespace-scoped Operator installiert wurde (Versionen 20.07 bis 20.10), sind die Upgrade-Anweisungen für die installierte Version von Trident zu verwenden.
...
.Schritte
. Wenn Sie link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-astra-trident-using-helm["Astra Trident mit Helm installiert"] haben, können Sie  `helm upgrade trident netapp-trident/trident-operator --version 23.10.0` verwenden, um das Upgrade in einem Schritt durchzuführen. Wenn Sie das Helm-Repository nicht hinzugefügt haben oder es nicht für das Upgrade verwenden können:
+
...
NOTE: Bevor zwischen den Installationsmethoden gewechselt wird, sollte Folgendes überprüft werden:link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["Wechseln zwischen den Installationsmethoden"]
